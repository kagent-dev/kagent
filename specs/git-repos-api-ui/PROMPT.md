...
- REST API (gorilla/mux, 8 endpoints under `/api/repos`) + unified MCP protocol (18 tools: 5 native + 13 Reflex) served from `serve` command
...
- Reflex (`reflex-search`) embedded as subprocess inside gitrepo-mcp: `rfx mcp` spawned via stdio, tools proxied through unified `/mcp` endpoint (replaces ast-grep). Agents get one MCP connection with 18 tools (5 native + 13 Reflex). No prefix — routing table dispatches by tool name.
...
- Given a cloned repo, when agent calls `search_code` via unified MCP, then matching results returned through Reflex subprocess proxy
- Given a cloned repo, when agent calls `get_dependencies` via unified MCP, then file import graph returned
- Given Reflex unavailable, when agent calls `tools/list`, then only 5 native tools returned (Reflex tools omitted)
