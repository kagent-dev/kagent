...
type ConfigurableRunner struct {
	...
	skillsTool      *adk.SkillsTool
	bashTool        *adk.BashTool
	fileTools       *adk.FileTools
	googleADKRunner *adk.GoogleADKRunnerWrapper // Wrapper around Google ADK Runner
	logger          logr.Logger
}

func NewConfigurableRunner(config *adk.AgentConfig, skillsDirectory string, logger logr.Logger) *ConfigurableRunner {
	...
	if skillsDirectory != "" {
		if _, err := os.Stat(skillsDirectory); err == nil {
			runner.skillsTool = adk.NewSkillsTool(skillsDirectory)
			runner.bashTool = adk.NewBashTool(skillsDirectory)
			runner.fileTools = &adk.FileTools{}
		}
	}
...
func (r *ConfigurableRunner) Run(ctx context.Context, args map[string]interface{}) (<-chan interface{}, error) {
	...
	if r.skillsDirectory != "" && sessionID != "" {
		if _, err := adk.InitializeSessionPath(sessionID, r.skillsDirectory); err != nil {
	...
	sessionService, _ := args[adk.ArgKeySessionService].(adk.SessionService)
...
func fallbackChannel(config *adk.AgentConfig) <-chan interface{} {
...
func fallbackChannelNoMessage(config *adk.AgentConfig) <-chan interface{} {
	ch := make(chan interface{}, 1)
	go func() {
		defer close(ch)
...
type ADKTaskManager struct {
	executor              *adk.A2aAgentExecutor
	taskStore             *core.KAgentTaskStore
	pushNotificationStore *core.KAgentPushNotificationStore
	logger                logr.Logger
}

func NewADKTaskManager(executor *adk.A2aAgentExecutor, taskStore *core.KAgentTaskStore, pushNotificationStore *core.KAgentPushNotificationStore, logger logr.Logger) taskmanager.TaskManager {
...
func NewTaskSavingEventQueue(inner adk.EventQueue, taskStore *core.KAgentTaskStore, taskID, contextID string, logger logr.Logger) *TaskSavingEventQueue {
...
func main() {
	...
	var tokenService *adk.KAgentTokenService
	if kagentURL != "" {
		tokenService = adk.NewKAgentTokenService(appName)
	...
	var sessionService adk.SessionService
	if kagentURL != "" {
		// Use token service for authenticated requests
		var httpClient *http.Client
		if tokenService != nil {
			httpClient = adk.NewHTTPClientWithToken(tokenService)
		} else {
			httpClient = &http.Client{Timeout: 30 * time.Second}
		}
		sessionService = adk.NewKAgentSessionServiceWithLogger(kagentURL, httpClient, logger)
	...
	executor := adk.NewA2aAgentExecutorWithLogger(runner, adk.A2aAgentExecutorConfig{
		Stream:           stream,
