...
func (r *MCPToolRegistry) FetchToolsFromHttpServer(ctx context.Context, config HttpMcpServerConfig) error {
...
func (r *MCPToolRegistry) FetchToolsFromSseServer(ctx context.Context, config SseMcpServerConfig) error {
	url := config.Params.Url
	headers := config.Params.Headers
	if headers == nil {
		headers = make(map[string]string)
