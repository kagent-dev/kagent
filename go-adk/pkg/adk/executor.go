...
func NewA2aAgentExecutorWithLogger(runner Runner, config A2aAgentExecutorConfig, sessionService SessionService, taskStore *core.KAgentTaskStore, appName string, logger logr.Logger) *A2aAgentExecutor {
...
func (e *A2aAgentExecutor) Execute(ctx context.Context, req *protocol.SendMessageParams, queue EventQueue, taskID, contextID string) error {
	...
	ctx = core.SetKAgentSpanAttributes(ctx, spanAttributes)
	defer func() {
		ctx = core.ClearKAgentSpanAttributes(ctx)
	}()
	...
	// 5. Start execution with timeout
	...
	aggregator := core.NewTaskResultAggregator()
	...
	for internalEvent := range eventChan {
		...
		isPartial := IsPartialEvent(internalEvent)

		a2aEvents := ConvertEventToA2AEvents(internalEvent, taskID, contextID, e.AppName, userID, sessionID)
		...
		// Append event to session: non-partial always; partial only when tool-related (function_call/function_response)
		// Report partial events for tool calls so session has tool activity during streaming.
		// Use background context with timeout so client disconnect does not cancel persist (events must reach DB).
		shouldAppend := !isPartial || EventHasToolContent(internalEvent)
		if e.SessionService != nil && session != nil && shouldAppend {
			appendCtx, appendCancel := context.WithTimeout(context.Background(), 30*time.Second)
			defer appendCancel()
			if err := e.SessionService.AppendEvent(appendCtx, session, internalEvent); err != nil {
...
func ExtractUserAndSessionID(req *protocol.SendMessageParams, contextID string) (userID, sessionID string) {
	...
	// if req.CallContext != nil && req.CallContext.User != nil {
	//     userID = req.CallContext.User.UserName
	// }
...
func (e *A2aAgentExecutor) sendFailure(ctx context.Context, queue EventQueue, taskID, contextID, message string) error {
	...
	if len(message) > 0 {
		...
		if mappedMsg := GetErrorMessage(message); mappedMsg != DefaultErrorMessage {
