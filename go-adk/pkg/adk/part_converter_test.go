...
func TestConvertA2APartToGenAIPart_TextPart(t *testing.T) {
	...
	result, err := ConvertA2APartToGenAIPart(textPart)
	if err != nil {
		t.Fatalf("ConvertA2APartToGenAIPart() error = %v", err)
	}

	if text, ok := result[PartKeyText].(string); !ok {
		t.Errorf("Expected %q key in result, got %v", PartKeyText, result)
...
func TestConvertA2APartToGenAIPart_FilePartWithURI(t *testing.T) {
	...
	result, err := ConvertA2APartToGenAIPart(filePart)
	if err != nil {
		t.Fatalf("ConvertA2APartToGenAIPart() error = %v", err)
	}

	fileData, ok := result[PartKeyFileData].(map[string]interface{})
	if !ok {
		t.Fatalf("Expected %q key in result, got %v", PartKeyFileData, result)
	}

	if uri, ok := fileData[PartKeyFileURI].(string); !ok || uri != "gs://bucket/file.png" {
		t.Errorf("Expected file_uri = %q, got %v", "gs://bucket/file.png", fileData[PartKeyFileURI])
	}

	if mime, ok := fileData[PartKeyMimeType].(string); !ok || mime != "image/png" {
		t.Errorf("Expected mime_type = %q, got %v", "image/png", fileData[PartKeyMimeType])
	}
}
...
func TestConvertA2APartToGenAIPart_FilePartWithBytes(t *testing.T) {
	...
	inlineData, ok := result[PartKeyInlineData].(map[string]interface{})
	if !ok {
		t.Fatalf("Expected %q key in result, got %v", PartKeyInlineData, result)
	}
	...
	if mime, ok := inlineData[PartKeyMimeType].(string); !ok || mime != "text/plain" {
		t.Errorf("Expected mime_type = %q, got %v", "text/plain", inlineData[PartKeyMimeType])
	}
}
...
func TestConvertA2APartToGenAIPart_DataPartFunctionCall(t *testing.T) {
	functionCallData := map[string]interface{}{
		PartKeyName: "search",
		PartKeyArgs: map[string]interface{}{
			"query": "test",
		},
	}
	...
	result, err := ConvertA2APartToGenAIPart(dataPart)
	if err != nil {
		t.Fatalf("ConvertA2APartToGenAIPart() error = %v", err)
	}

	if functionCall, ok := result[PartKeyFunctionCall].(map[string]interface{}); !ok {
		t.Errorf("Expected %q key in result, got %v", PartKeyFunctionCall, result)
	} else {
...
func TestConvertA2APartToGenAIPart_DataPartFunctionResponse(t *testing.T) {
	...
	if functionResponse, ok := result[PartKeyFunctionResponse].(map[string]interface{}); !ok {
		t.Errorf("Expected %q key in result, got %v", PartKeyFunctionResponse, result)
	} else {
...
func TestConvertGenAIPartToA2APart_FilePartWithURI(t *testing.T) {
	genaiPart := map[string]interface{}{
		PartKeyFileData: map[string]interface{}{
...
func TestConvertGenAIPartToA2APart_FilePartWithBytes(t *testing.T) {
	testData := []byte("test file content")
	genaiPart := map[string]interface{}{
		PartKeyInlineData: map[string]interface{}{
			"data":           testData,
...
func TestConvertGenAIPartToA2APart_FunctionCall(t *testing.T) {
	genaiPart := map[string]interface{}{
		PartKeyFunctionCall: map[string]interface{}{
			PartKeyName: "search",
			PartKeyArgs: map[string]interface{}{
				"query": "test",
			},
...
func TestConvertGenAIPartToA2APart_FunctionResponse(t *testing.T) {
	genaiPart := map[string]interface{}{
		PartKeyFunctionResponse: map[string]interface{}{
			"result": "search results",
		},
	}
...
func TestConvertGenAIPartToA2APart_FunctionResponseMCPContent(t *testing.T) {
	...
	genaiPart := map[string]interface{}{
		PartKeyFunctionResponse: map[string]interface{}{
			...
			PartKeyResponse: map[string]interface{}{
				"content": contentArr,
			},
		},
	}
	...
	resp, ok := data[PartKeyResponse].(map[string]interface{})
	if !ok {
