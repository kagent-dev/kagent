...
func extractRunArgs(args map[string]interface{}) runArgs {
	...
	if svc, ok := args[ArgKeySessionService].(SessionService); ok {
		r.sessionService = svc
	}
	if s, ok := args[ArgKeySession].(*Session); ok {
		r.session = s
...
func runConfigFromArgs(args map[string]interface{}) agent.RunConfig {
	...
	if m, ok := args[ArgKeyRunConfig].(map[string]interface{}); ok {
		if stream, ok := m[RunConfigKeyStreamingMode].(string); ok && stream == "SSE" {
...
func (w *GoogleADKRunnerWrapper) Run(ctx context.Context, args map[string]interface{}) (<-chan interface{}, error) {
	...
	go func() {
		...
		if contentErr != nil {
			...
			ch <- &Event{
				ID: uuid.New().String(), Author: "system", Timestamp: time.Now().UTC().Format(time.RFC3339),
		...
		for adkEvent, err := range eventSeq {
			...
			if ctx.Err() != nil {
				...
				ch <- &Event{
					...
					ErrorMessage: fmt.Sprintf("Google ADK runner timed out or was cancelled: %v", ctx.Err()),
				}
				return
			}
			if err != nil {
...
func hasNonEmptyParts(msgMap map[string]interface{}) bool {
	partsVal, exists := msgMap[PartKeyParts]
...
func convertMapToGenAIContent(msgMap map[string]interface{}) (*genai.Content, error) {
	role, _ := msgMap[PartKeyRole].(string)
	if role == "" {
		role = "user"
	}
	...
	if partsVal, exists := msgMap[PartKeyParts]; exists && partsVal != nil {
		if partsList, ok := partsVal.([]interface{}); ok {
	...
	for _, partInterface := range partsInterface {
		if partMap, ok := partInterface.(map[string]interface{}); ok {
			// Handle text parts
			if text, ok := partMap[PartKeyText].(string); ok {
				parts = append(parts, genai.NewPartFromText(text))
				continue
			}
			// Handle function calls
			if functionCall, ok := partMap[PartKeyFunctionCall].(map[string]interface{}); ok {
				name, _ := functionCall[PartKeyName].(string)
				args, _ := functionCall[PartKeyArgs].(map[string]interface{})
				if name != "" {
					genaiPart := genai.NewPartFromFunctionCall(name, args)
					if id, ok := functionCall[PartKeyID].(string); ok && id != "" {
						genaiPart.FunctionCall.ID = id
					}
					parts = append(parts, genaiPart)
				}
			...
			if functionResponse, ok := partMap[PartKeyFunctionResponse].(map[string]interface{}); ok {
				name, _ := functionResponse[PartKeyName].(string)
				response, _ := functionResponse[PartKeyResponse].(map[string]interface{})
				if name != "" {
					genaiPart := genai.NewPartFromFunctionResponse(name, response)
					if id, ok := functionResponse[PartKeyID].(string); ok && id != "" {
						genaiPart.FunctionResponse.ID = id
					}
					parts = append(parts, genaiPart)
				}
			...
			// Handle file_data
			if fileData, ok := partMap[PartKeyFileData].(map[string]interface{}); ok {
			...
			// Handle inline_data
			if inlineData, ok := partMap[PartKeyInlineData].(map[string]interface{}); ok {
				var data []byte
