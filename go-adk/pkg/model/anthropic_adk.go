// Package model: Anthropic model implementing Google ADK model.LLM using genai types.
package model
...
func runAnthropicStreaming(ctx context.Context, m *AnthropicModel, params anthropic.MessageNewParams, yield func(*adkmodel.LLMResponse, error) bool) {
		...
		case anthropic.ContentBlockStartEvent:
			...
			if e.ContentBlock.Type == "tool_use" {
				if toolUse, ok := e.ContentBlock.AsAny().(anthropic.ToolUseBlock); ok {
					toolUseBlocks[blockIndex] = struct {
						id        string
						name      string
						inputJSON string
		...
		case anthropic.ContentBlockDeltaEvent:
			...
			case "input_json_delta":
				if jsonDelta, ok := delta.AsAny().(anthropic.InputJSONDelta); ok {
					if block, exists := toolUseBlocks[blockIndex]; exists {
						block.inputJSON += jsonDelta.PartialJSON
						toolUseBlocks[blockIndex] = block
					}
				}
			}
	...
	if err := stream.Err(); err != nil {
		...
		_ = yield(&adkmodel.LLMResponse{ErrorCode: "STREAM_ERROR", ErrorMessage: err.Error()}, nil)
		return
	}
...
func runAnthropicNonStreaming(ctx context.Context, m *AnthropicModel, params anthropic.MessageNewParams, yield func(*adkmodel.LLMResponse, error) bool) {
