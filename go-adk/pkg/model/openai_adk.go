// Package model: OpenAI model implementing Google ADK model.LLM using genai types only.
package model
...
func runStreaming(ctx context.Context, m *OpenAIModel, params openai.ChatCompletionNewParams, yield func(*adkmodel.LLMResponse, error) bool) {
...
func runNonStreaming(ctx context.Context, m *OpenAIModel, params openai.ChatCompletionNewParams, yield func(*adkmodel.LLMResponse, error) bool) {
	...
	if len(completion.Choices) == 0 {
		yield(&adkmodel.LLMResponse{ErrorCode: "API_ERROR", ErrorMessage: "No choices in response"}, nil)
		return
	}
