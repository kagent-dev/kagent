...
func newOpenAIModelFromConfig(ctx context.Context, config *OpenAIConfig, apiKey string) (*OpenAIModel, error) {
	...
	if len(config.Headers) > 0 {
		...
		log.Info("Setting default headers for OpenAI client", "headersCount", len(config.Headers), "headers", config.Headers)
	}
	opts = append(opts, option.WithHTTPClient(httpClient))

	client := openai.NewClient(opts...)
	log.Info("Initialized OpenAI model", "model", config.Model, "baseUrl", config.BaseUrl)
	return &OpenAIModel{
		Config:  config,
		Client:  client,
		IsAzure: false,
...
func NewAzureOpenAIModel(ctx context.Context, config *AzureOpenAIConfig) (*OpenAIModel, error) {
	...
	client := openai.NewClient(opts...)
	log.Info("Initialized Azure OpenAI model", "model", config.Model, "endpoint", azureEndpoint, "apiVersion", apiVersion)
	return &OpenAIModel{
