...
E2E_PLACEHOLDER_DIGEST ?= sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
...
build-controller-e2e: ## Build the E2E controller with placeholder digests for unused sandbox workloads
build-controller-e2e: buildx-create
	@set -e; \
	DIGEST_LDFLAGS=$$(CONTAINER_RUNTIME=$(CONTAINER_RUNTIME) \
		APP_IMG=$(APP_IMG)@$(E2E_PLACEHOLDER_DIGEST) \
		APP_FULL_IMG=$(APP_FULL_IMG)@$(E2E_PLACEHOLDER_DIGEST) \
		GOLANG_ADK_IMG=$(GOLANG_ADK_IMG)@$(E2E_PLACEHOLDER_DIGEST) \
		GOLANG_ADK_FULL_IMG=$(GOLANG_ADK_FULL_IMG)@$(E2E_PLACEHOLDER_DIGEST) \
		ACP_SANDBOX_OPENCLAW_IMG=$(ACP_SANDBOX_OPENCLAW_IMG)@$(E2E_PLACEHOLDER_DIGEST) \
		ACP_SANDBOX_HERMES_IMG=$(ACP_SANDBOX_HERMES_IMG)@$(E2E_PLACEHOLDER_DIGEST) \
		./scripts/controller-digest-ldflags.sh); \
	$(DOCKER_BUILDER) $(DOCKER_BUILD_ARGS) $(TOOLS_IMAGE_BUILD_ARGS) \
		$(call docker-cache-args,controller) \
		--build-arg LDFLAGS="$(LDFLAGS)$$DIGEST_LDFLAGS" \
...
build-e2e: buildx-create build-skills-init build-app build-app-full build-golang-adk build-golang-adk-full build-controller-e2e
...
push-test-agent: build-kagent-adk push-test-agent-images ## Build and push E2E test agent images to the local registry

.PHONY: build-test-kebab
build-test-kebab: buildx-create build-kagent-adk-full
...
.PHONY: build-test-poem-flow
build-test-poem-flow: buildx-create
...
.PHONY: build-test-basic-openai
build-test-basic-openai: buildx-create
...
.PHONY: build-test-langgraph-kebab
build-test-langgraph-kebab: buildx-create
...
.PHONY: build-test-agent-images
build-test-agent-images: build-test-kebab build-test-poem-flow build-test-basic-openai build-test-langgraph-kebab

.PHONY: push-test-agent-images
push-test-agent-images: build-test-agent-images ## Build, push, and deploy E2E test agents
...
.PHONY: build-test-kebab-maker
build-test-kebab-maker: buildx-create
...
push-test-skill: build-test-kebab-maker ## Build and push E2E test skill images to the local registry
...
.PHONY: ci-e2e-install
ci-e2e-install: ## Build all E2E images in parallel, install kagent, and deploy the test agent
ci-e2e-install: helm-install-e2e build-test-agent-images build-test-kebab-maker
	kubectl apply --namespace kagent --context kind-$(KIND_CLUSTER_NAME) -f go/core/test/e2e/agents/kebab/agent.yaml
