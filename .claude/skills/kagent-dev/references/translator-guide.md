...
Here's a complete example of adding a `Replicas` field that affects the Deployment:
...
type DeclarativeAgentSpec struct {
    ...
    // Replicas is the number of agent pod replicas
    // +optional
...
func (a *adkApiTranslator) translateInlineAgent(...) {
    ...
    // Set replicas from CRD or default to 1
    replicas := int32(1)
...
**3. Add test fixture:**
```yaml
# go/core/internal/controller/translator/agent/testdata/inputs/agent_with_replicas.yaml
...
**5. Verify golden file:**
```bash
# Check that replicas appears in output
jq '.deployment.spec.replicas' go/core/internal/controller/translator/agent/testdata/outputs/agent_with_replicas.json
# Should output: 3
```

**6. Add E2E test:**
```go
func TestE2EAgentWithReplicas(t *testing.T) {
    agent := &v1alpha2.Agent{
        ObjectMeta: metav1.ObjectMeta{
            Name:      "test-replicas-" + randString(),
            Namespace: "kagent",
        },
        Spec: v1alpha2.AgentSpec{
            Declarative: &v1alpha2.DeclarativeAgentSpec{
    ...
    // Verify deployment has 3 replicas
    require.Eventually(t, func() bool {
        deployment := &appsv1.Deployment{}
        err := k8sClient.Get(ctx, types.NamespacedName{
            Name:      agent.Name,
