...
def detect_test_relationships(
    ...
    # Build indexes — index by both unqualified name and scope.name so
    # test_Class_method patterns can match qualified "Class.method" keys.
...
def _resolve_relative_import(
    src_file: str, module_name: str, known_files: set[str],
    js_project_roots: list[str],
) -> Optional[str]:
    """Resolve a relative JS/TS import to an actual file path.

    Handles ./foo, ../bar (filesystem-relative) and @/lib/utils
    (Next.js src/ alias).
    """
    if module_name.startswith("@/"):
        # Next.js / TS path alias: @/ maps to src/ within the project root.
        # Find the deepest JS project root that is a parent of src_file.
        project_root = ""
        for root in js_project_roots:
            prefix = root + "/" if root else ""
            if src_file.startswith(prefix) or not root:
                if len(root) > len(project_root):
                    project_root = root
        if project_root:
            rel_import = module_name[2:]  # strip @/
            base = str(Path(project_root) / "src" / rel_import)
        else:
            return None
    else:
        # Standard relative: resolve against the source file's directory
        src_dir = str(Path(src_file).parent)
        base = os.path.normpath(os.path.join(src_dir, module_name))
...
def resolve_imports(
    ...
    for boundary in module_boundaries:
        if boundary.language == "go" and boundary.name:
            ...
            go_module_root = boundary.root_dir  # e.g. "go"
            break
    ...
    # Build a set of known file paths for O(1) lookup during relative resolution
    known_files = set(file_map.keys())

    # Collect JS/TS project roots from module boundaries (package.json locations)
    js_project_roots = [
        mb.root_dir for mb in module_boundaries
        if mb.build_file == "package.json"
    ]
    ...
    for edge in edges:
        ...
        if target.startswith("module:"):
            ...
            # Resolve relative JS/TS imports (./foo, ../bar, @/lib/utils)
            if module_name.startswith("./") or module_name.startswith("../") or module_name.startswith("@/"):
                src_file = edge.source[5:] if edge.source.startswith("file:") else ""
                if src_file:
                    rel_resolved = _resolve_relative_import(src_file, module_name, known_files, js_project_roots)
                    if rel_resolved:
            ...
            # e.g. "github.com/kagent-dev/kagent/go/adk/pkg/a2a"
            #   -> strip prefix "github.com/kagent-dev/kagent/go"
            #   -> remainder "/adk/pkg/a2a" -> "adk/pkg/a2a"
            #   -> prepend module root "go" -> "go/adk/pkg/a2a"
            if go_module_prefix and module_name.startswith(go_module_prefix):
                remainder = module_name[len(go_module_prefix):].lstrip("/")
                if go_module_root:
                    local_path = f"{go_module_root}/{remainder}" if remainder else go_module_root
                else:
            ...
            # Try Go package mapping by suffix match on path segments
            for pkg_path, pkg_files in go_pkg_map.items():
                # Compare by path suffix (slash-separated), not dotted
                if module_name.endswith("/" + pkg_path) or module_name.endswith(pkg_path):
                    for f in pkg_files:
                        resolved.append(Edge(edge.source, f"file:{f}", "imports", edge.metadata))
                    break
            else:
...
def main():
    ...
    if args.incremental and prev_graph is not None and changed_files:
        ...
        for edge_data in prev_edges:
            ...
            # Drop edges whose SOURCE is a changed file (reparsing will
            # regenerate them).  Keep edges from unchanged files even if
            # they point TO a changed file — those importers/callers are
            # not reparsed, so their outgoing edges must be preserved.
    ...
    # Resolve calls BEFORE import resolution so the alias map (built from
    # module:/symbol: import edges) is still intact.
    ...
    # Resolve imports (rewrites module:/symbol: edges to file: edges)
