...
func TestToolServerController(t *testing.T) {
	t.Run("When reconciling a resource", func(t *testing.T) {
		...
		toolserver := &agentv1alpha1.ToolServer{}

		// Setup - creating the custom resource for the Kind ToolServer
		err := k8sClient.Get(ctx, typeNamespacedName, toolserver)
		if err != nil && errors.IsNotFound(err) {
			resource := &agentv1alpha1.ToolServer{
		...
		t.Cleanup(func() {
			...
			// Cleanup the specific resource instance ToolServer
