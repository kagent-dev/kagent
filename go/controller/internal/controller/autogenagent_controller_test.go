...
func TestAutogenAgentController(t *testing.T) {
	t.Run("When reconciling a resource", func(t *testing.T) {
		...
		autogenagent := &agentv1alpha1.Agent{}

		// Setup - creating the custom resource for the Kind AutogenAgent
		err := k8sClient.Get(ctx, typeNamespacedName, autogenagent)
		...
		t.Cleanup(func() {
			...
			// Cleanup the specific resource instance AutogenAgent
		...
		t.Run("should successfully reconcile the resource", func(t *testing.T) {
			...
			controllerReconciler := &AutogenAgentReconciler{
				Client: k8sClient,
