...
func ReconcileMCPServerStatus(ctx context.Context, kube client.Client, mcpServer *v1alpha2.MCPServer, reconcileErr error) (bool, error) {
	...
	setResolvedRefsCondition(mcpServer, true, v1alpha2.MCPServerReasonResolvedRefs, "All references resolved")
...
func setAcceptedCondition(mcpServer *v1alpha2.MCPServer, err error) {
	status := metav1.ConditionTrue
	reason := v1alpha2.MCPServerReasonAccepted
	message := "MCPServer has been accepted for processing"

	if err != nil {
		status = metav1.ConditionFalse
		reason = v1alpha2.MCPServerReasonInvalidConfig
		message = fmt.Sprintf("MCPServer configuration is invalid: %v", err)
	}

	setCondition(mcpServer, v1alpha2.MCPServerConditionAccepted, status, reason, message)
}

// setResolvedRefsCondition sets the ResolvedRefs condition on the MCPServer
func setResolvedRefsCondition(mcpServer *v1alpha2.MCPServer, resolved bool, reason v1alpha2.MCPServerConditionReason, message string) {
	status := metav1.ConditionTrue
	if !resolved {
		status = metav1.ConditionFalse
	}
	setCondition(mcpServer, v1alpha2.MCPServerConditionResolvedRefs, status, reason, message)
}

// setProgrammedCondition sets the Programmed condition on the MCPServer
func setProgrammedCondition(mcpServer *v1alpha2.MCPServer, programmed bool) {
	status := metav1.ConditionTrue
	reason := v1alpha2.MCPServerReasonProgrammed
	message := "MCPServer has been successfully programmed"

	if !programmed {
		status = metav1.ConditionFalse
		reason = v1alpha2.MCPServerReasonDeploymentFailed
		message = "Failed to program MCPServer resources"
	}

	setCondition(mcpServer, v1alpha2.MCPServerConditionProgrammed, status, reason, message)
}
...
func checkReadyCondition(ctx context.Context, kube client.Client, mcpServer *v1alpha2.MCPServer) bool {
	...
	if err := kube.Get(ctx, types.NamespacedName{Name: deploymentName, Namespace: mcpServer.Namespace}, deployment); err != nil {
		if client.IgnoreNotFound(err) == nil {
			setReadyCondition(mcpServer, false, v1alpha2.MCPServerReasonPodsNotReady, "Deployment not found")
		} else {
			setReadyCondition(
				mcpServer,
				false,
				v1alpha2.MCPServerReasonPodsNotReady,
	...
	if deployment.Status.AvailableReplicas > 0 && deployment.Status.AvailableReplicas == deployment.Status.Replicas {
		setReadyCondition(
			mcpServer,
			true,
			v1alpha2.MCPServerReasonAvailable,
...
func setReadyCondition(mcpServer *v1alpha2.MCPServer, ready bool, reason v1alpha2.MCPServerConditionReason, message string) {
	status := metav1.ConditionTrue
	if !ready {
		status = metav1.ConditionFalse
	}
	setCondition(mcpServer, v1alpha2.MCPServerConditionReady, status, reason, message)
}

// setCondition sets the given condition on the MCPServer status
func setCondition(
	mcpServer *v1alpha2.MCPServer,
	conditionType v1alpha2.MCPServerConditionType,
	status metav1.ConditionStatus,
	reason v1alpha2.MCPServerConditionReason,
	message string,
) {
	now := metav1.Now()
...
func updateMCPServerStatus(ctx context.Context, kube client.Client, mcpServer *v1alpha2.MCPServer) error {
