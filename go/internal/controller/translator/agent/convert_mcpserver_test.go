		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if tt.want.TLS == nil {
					...
					assert.Equal(t, tt.want.TLS.SecretRef, got.TLS.SecretRef, "TLS SecretRef should match")
					assert.Equal(t, tt.want.TLS.InsecureSkipVerify, got.TLS.InsecureSkipVerify, "TLS InsecureSkipVerify should match")
				}
			}
		})
	}
