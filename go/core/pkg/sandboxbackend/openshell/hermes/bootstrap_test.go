...
func TestBuildBootstrapArtifacts_TelegramSlack(t *testing.T) {
	...
	env := string(envFile)
	require.Contains(t, env, "TELEGRAM_BOT_TOKEN=openshell:resolve:env:TELEGRAM_BOT_TOKEN")
	require.Contains(t, env, "TELEGRAM_ALLOWED_USERS=123456789")
	require.Contains(t, env, "SLACK_BOT_TOKEN=xoxb-OPENSHELL-RESOLVE-ENV-SLACK_BOT_TOKEN")
	require.Contains(t, env, "SLACK_APP_TOKEN=xapp-OPENSHELL-RESOLVE-ENV-SLACK_APP_TOKEN")
	require.Contains(t, env, "SLACK_ALLOWED_USERS=U01234567,U89ABCDEF")
	require.Contains(t, env, "SLACK_HOME_CHANNEL=C01234567890")
	require.Contains(t, env, "SLACK_HOME_CHANNEL_NAME=general")
	require.Equal(t, "tg-token", execEnv["TELEGRAM_BOT_TOKEN"])
	require.Equal(t, "xoxb-bot", execEnv["SLACK_BOT_TOKEN"])
}
