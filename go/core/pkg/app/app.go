...
func (cfg *Config) SetFlags(commandLine *flag.FlagSet) {
	...
	commandLine.StringVar(&agent_translator.PythonADKFullImageDigestOverride, "app-full-image-digest", "", "Explicit manifest digest (sha256:...) for the full Python agent runtime image (skills/SRT). Sandbox agents fall back to the link-time digest when this is unset. Declarative skills agents pin the full variant only when this is set (IMAGE_TAG embedding a digest is not enough, because released builds always bake an upstream digest).")
	...
	commandLine.StringVar(&agent_translator.GoADKFullImageDigestOverride, "golang-adk-full-image-digest", "", "Explicit manifest digest (sha256:...) for the full Go agent runtime image (skills/SRT). Same override rules as --app-full-image-digest.")
