...
CREATE TABLE agent_instance_task (
    ...
    turn_phase             TEXT,
    turn_request           BYTEA,
    turn_previous_task     BYTEA,
    turn_owner_id          UUID,
    turn_owner_expires_at  TIMESTAMPTZ,
    turn_cancel_requested  BOOLEAN     NOT NULL DEFAULT FALSE,
    position               BIGINT      NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    PRIMARY KEY (history_id, id),
    CONSTRAINT agent_instance_task_turn_phase_check CHECK (
        turn_phase IS NULL OR turn_phase IN ('ADMITTED', 'ISSUED', 'CANCELING', 'QUIESCING', 'SETTLED')
    ),
    CONSTRAINT agent_instance_task_turn_identity_check CHECK (
        (turn_id IS NULL AND turn_phase IS NULL AND turn_request IS NULL AND turn_previous_task IS NULL
            AND turn_cancel_requested = FALSE)
        OR (turn_id IS NOT NULL AND turn_phase IS NOT NULL AND turn_request IS NOT NULL)
    ),
    CONSTRAINT agent_instance_task_turn_owner_lease_check CHECK (
        (turn_owner_id IS NULL AND turn_owner_expires_at IS NULL)
        OR (turn_owner_id IS NOT NULL AND turn_owner_expires_at IS NOT NULL)
    ),
    CONSTRAINT agent_instance_task_turn_owned_phase_check CHECK (
        turn_owner_id IS NULL
        OR (turn_phase IS NOT NULL AND turn_phase IN ('ADMITTED', 'ISSUED', 'CANCELING', 'QUIESCING'))
    ),
    CONSTRAINT agent_instance_task_turn_effect_owner_check CHECK (
        turn_phase IS NULL OR turn_phase NOT IN ('ISSUED', 'CANCELING', 'QUIESCING') OR turn_owner_id IS NOT NULL
    ),
    CONSTRAINT agent_instance_task_turn_settled_owner_check CHECK (
        turn_phase IS DISTINCT FROM 'SETTLED' OR turn_owner_id IS NULL
    ),
    CONSTRAINT agent_instance_task_turn_canceling_intent_check CHECK (
        turn_phase IS DISTINCT FROM 'CANCELING' OR turn_cancel_requested
    ),
    CONSTRAINT agent_instance_task_turn_cancel_identity_check CHECK (
        turn_cancel_requested = FALSE OR turn_id IS NOT NULL
    )
);
