...
const (
	defaultScheduledRunExecutionPageSize = 50
	maxScheduledRunExecutionPageSize     = 100
)
...
func (h *ScheduledRunsHandler) HandleListScheduledRunExecutions(w ErrorResponseWriter, r *http.Request) {
	...
	options := database.ScheduledRunExecutionQueryOptions{Limit: defaultScheduledRunExecutionPageSize}
...
func (h *ScheduledRunsHandler) HandleUpdateScheduledRun(w ErrorResponseWriter, r *http.Request) {
	...
	if err := h.KubeClient.Get(r.Context(), key, &v1alpha2.ScheduledRun{}); err != nil {
		if apierrors.IsNotFound(err) {
			w.RespondWithError(errors.NewNotFoundError("Scheduled Run not found", err))
			return
		}
...
func (h *ScheduledRunsHandler) HandleTriggerScheduledRun(w ErrorResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if stderrors.Is(err, scheduledrun.ErrSchedulerNotActive) {
			w.RespondWithError(errors.NewServiceUnavailableError("Scheduled Run scheduler is starting or shutting down", err))
			return
		}
