...
func (m *mockScheduledRunTrigger) TriggerManualRun(key types.NamespacedName) {
...
func TestScheduledRunsHandler(t *testing.T) {
	...
	t.Run("HandleCreateScheduledRun", func(t *testing.T) {
		...
		t.Run("invalid schedule - too frequent", func(t *testing.T) {
			handler, _, w := setupHandler()
...
func TestValidateScheduleFrequency(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "invalid - every 5 minutes",
			schedule: "*/5 * * * *",
			wantErr:  true,
		},
		{
			name:     "invalid - every 30 minutes",
			schedule: "*/30 * * * *",
			wantErr:  true,
		},
		{
