...
func TestHandleListAgents(t *testing.T) {
	...
	t.Run("includes openshell AgentHarness CR in agent list", func(t *testing.T) {
		...
		for _, row := range response.Data {
			if row.AgentHarness == nil {
				continue
			}
			found = true
			require.Equal(t, "default-openclaw-1", row.AgentHarness.GatewaySandboxName)
...
func TestHandleCreateAgentHarness(t *testing.T) {
	t.Run("creates openclaw AgentHarness", func(t *testing.T) {
		...
		require.NotNil(t, response.Data.AgentHarness)
		require.Equal(t, v1alpha2.AgentHarnessBackendOpenClaw, response.Data.AgentHarness.Backend)
