...
func TestA2AProtobufTaskEventScope(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NoError(t, q.UpsertAgentInstanceTask(ctx, dbgen.UpsertAgentInstanceTaskParams{HistoryID: contextID, ID: original.Id, State: string(a2a.TaskStateWorking), Data: data}))
