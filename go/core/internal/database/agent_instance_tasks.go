...
func (c *Client) writeRuntimeTask(ctx context.Context, instanceID string, expectedVersion int64, mutationHash []byte, task *a2a.Task, event a2a.Event) (int64, error) {
	...
	err := c.withTx(ctx, func(tx pgx.Tx) error {
		...
		if expectedVersion == 0 && boundary {
			...
			if err := storeAgentInstanceTaskEvent(ctx, tx, instance, &initial, &initial, nil, false); err != nil {
				return err
			}
		}
...
func saveTaskProjection(ctx context.Context, db dbExecutor, historyID uuid.UUID, taskID, state string, statusTimestamp *time.Time, data []byte) (agentInstanceTaskRow, error) {
	return queryOne(ctx, db, `
		...
		ON CONFLICT (history_id, id) DO UPDATE SET
		    ...
		    data = EXCLUDED.data,
		    updated_at = NOW()
		RETURNING history_id, id, state, status_timestamp, data, created_at, updated_at, snapshot_atespace, snapshot_uri, snapshot_content_scope, history_sequence, position
