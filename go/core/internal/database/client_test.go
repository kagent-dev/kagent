...
func TestListUserTasks(t *testing.T) {
	...
	// prove the JSON cast matches both row shapes. It carries no user_id, so it
	// also covers the NULL-owner fallback: s1 resolves to alice alone.
	...
	// A task owned by bob but parked in alice's session. The session join alone
	// would hand it to alice; the task-owner predicate keeps it out.
	foreignData, err := json.Marshal(mkTask("t6", "s1", a2a.TaskStateWorking, late))
	require.NoError(t, err)
	_, err = db.Exec(ctx,
		`INSERT INTO task (id, data, session_id, user_id, created_at, updated_at)
		 VALUES ($1, $2, $3, $4, NOW(), NOW())`,
		"t6", string(foreignData), "s1", "bob")
	require.NoError(t, err)
	...
	t.Run("cross-user isolation", func(t *testing.T) {
		...
		require.Equal(t, []string{"t4"}, ids(tasks), "t6 sits in a session bob does not own")
	})

	t.Run("foreign task in own session stays hidden", func(t *testing.T) {
		tasks, _, err := client.ListUserTasks(ctx, dbpkg.ListUserTasksParams{UserID: "alice", SessionID: "s1", Limit: 50})
		require.NoError(t, err)
