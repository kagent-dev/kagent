...
func TestScheduledRunRequestsSurviveEditAndDeletion(t *testing.T) {
	...
	require.Equal(t, execution.Id, history[0].Id)
	page, err := c.ListScheduledRunExecutions(t.Context(), ScheduledRunExecutionQuery{ScheduledRunQuery: ScheduledRunQuery{Creator: "alice", AfterID: execution.Id, Limit: 1}, ScheduledRunID: schedule.Id})
	require.NoError(t, err)
