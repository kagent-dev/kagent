...
func TestSessionOperationGenerationAndTombstone(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, apiv1alpha1.RuntimeOperation_RUNTIME_OPERATION_CREATE, create.Session.Operation)
	creator := uuid.New()
	...
	require.NoError(t, err)
	require.Equal(t, apiv1alpha1.RuntimeOperation_RUNTIME_OPERATION_SUSPEND, suspend.Session.Operation)
	executor := uuid.New()
	...
	require.NoError(t, err)
	require.Equal(t, apiv1alpha1.RuntimeOperation_RUNTIME_OPERATION_RESUME, resume.Session.Operation)
	resumer := uuid.New()
	...
	require.NoError(t, err)
	require.Equal(t, apiv1alpha1.RuntimeOperation_RUNTIME_OPERATION_DELETE, deletion.Session.Operation)
	deleter := uuid.New()
...
func TestLifecycleObservationUsesCurrentFields(t *testing.T) {
	...
	require.Equal(t, apiv1alpha1.RuntimeOperation_RUNTIME_OPERATION_NONE, resumed.Session.Operation)
