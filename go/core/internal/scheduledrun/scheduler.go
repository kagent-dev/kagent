...
type ScheduledRunScheduler struct {
	...
	// shutdown unwinds all leader-owned work. Start runs only on the elected
	// leader (see NeedLeaderElection); manual dispatches may run on any API replica.
	managerCtx atomic.Pointer[context.Context]
	// active reports whether this replica currently owns the leader-only scheduler
	// lifecycle. It gates outcome polling, not manual dispatch.
	active atomic.Bool
...
// NeedLeaderElection makes the scheduler a leader-only Runnable so that a
// multi-replica deployment dispatches each execution exactly once, rather than
// once per replica.
func (s *ScheduledRunScheduler) NeedLeaderElection() bool { return true }
...
func (s *ScheduledRunScheduler) UpdateCronEntry(sr *v1alpha2.ScheduledRun) error {
	...
	if IsSuspended(sr) {
		metrics.SetActiveSchedules(len(s.cronEntries))
		return nil
	}
...
// Manual executions may be dispatched by any API replica; only cron dispatch and
// outcome polling require the leader-only scheduler lifecycle to be active.
...
func (s *ScheduledRunScheduler) executeOnce(ctx context.Context, key types.NamespacedName, manual bool) (*v1alpha2.ScheduledRunExecution, error) {
	...
	// Persist ownership before dispatch. The target's ADK reads the session while
	// handling SendMessage, and ScheduledRun session authorization resolves that
	// ownership through the durable execution record.
	pendingExecution := v1alpha2.ScheduledRunExecution{
		ID:        executionID,
		StartTime: startTime,
		Trigger:   trigger,
	...
	if err := s.storeExecution(context.Background(), key, string(sr.UID), pendingExecution); err != nil {
		return &pendingExecution, fmt.Errorf("failed to persist execution before dispatch for %s: %w", key, err)
	}
	...
	metrics.ObserveScheduledRunDispatch(string(execution.Status), time.Since(dispatchStart).Seconds())
	if dispatchErr != nil {
	...
	statusErr := s.updateStatusWithRetry(writeCtx, key, func(latest *v1alpha2.ScheduledRun) {
		mergeExecutionStatus(latest, execution)
	...
	// A manual execution can be dispatched by a follower API replica, but outcome
	// pollers must remain leader-owned. The status transition to InProgress causes
	// the ScheduledRun controller on the leader to reconcile and attach the poller.
	if s.active.Load() && execution.Status == v1alpha2.ScheduledRunExecutionStatus_InProgress && s.outcomePollerHook != nil {
...
func (s *ScheduledRunScheduler) dispatchToTarget(ctx context.Context, sr *v1alpha2.ScheduledRun, sessionID string) (a2atype.SendMessageResult, error) {
	...
	source := database.SessionSourceScheduledRun
...
// mergeExecutionStatus upserts one execution, keeps LastExecutionTime monotonic,
// and normalizes the bounded status summary. Status writers may complete in any
// order because overlapping executions are allowed.
func mergeExecutionStatus(sr *v1alpha2.ScheduledRun, execution v1alpha2.ScheduledRunExecution) {
	found := false
	for i := range sr.Status.RecentExecutions {
		if sr.Status.RecentExecutions[i].ID == execution.ID {
			existing := sr.Status.RecentExecutions[i]
			if execution.StartTime.IsZero() {
				execution.StartTime = existing.StartTime
			}
			if execution.SessionID == nil {
				execution.SessionID = existing.SessionID
			}
			if execution.TaskID == nil {
				execution.TaskID = existing.TaskID
			}
			if execution.Trigger == "" {
				execution.Trigger = existing.Trigger
			}
	...
	if sr.Status.LastExecutionTime == nil || execution.StartTime.After(sr.Status.LastExecutionTime.Time) {
		startTime := execution.StartTime
		sr.Status.LastExecutionTime = &startTime
	}
	trimRecentExecutions(sr)
}

// trimRecentExecutions keeps all in-progress executions so their pollers can
// resume after a restart, plus the configured number of newest completed
// summary executions. Sorting makes trimming independent of dispatch and
// completion order.
func trimRecentExecutions(sr *v1alpha2.ScheduledRun) {
	slices.SortFunc(sr.Status.RecentExecutions, func(a, b v1alpha2.ScheduledRunExecution) int {
		if byStartTime := a.StartTime.Compare(b.StartTime.Time); byStartTime != 0 {
			return byStartTime
		}
