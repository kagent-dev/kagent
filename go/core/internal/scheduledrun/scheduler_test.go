...
func TestTriggerManualExecutionRequiresActiveScheduler(t *testing.T) {
	s := newTestScheduledRunScheduler(t, nil)
	_, err := s.TriggerManualExecution(context.Background(), types.NamespacedName{Namespace: "default", Name: "execution"})
	assert.ErrorIs(t, err, ErrSchedulerNotActive)
}
...
func TestSchedulerStartActivatesAndDrains(t *testing.T) {
	...
	// The scheduler does not opt into leader election, so the manager runs it on
	// every replica.
	_, isLeaderAware := any(s).(manager.LeaderElectionRunnable)
	assert.False(t, isLeaderAware)
...
func TestExecuteOnce_PersistsDurableExecutionHistory(t *testing.T) {
	...
	assert.Equal(t, "default", db.executionRecords[0].ScheduledRunNamespace)
	assert.Equal(t, "durable", db.executionRecords[0].ScheduledRunName)
	assert.Equal(t, execution.Status, db.executionRecords[0].Status)
	assert.Equal(t, execution.SessionID, db.executionRecords[0].SessionID)
	assert.Equal(t, v1alpha2.ScheduledRunExecutionTrigger_Manual, db.executionRecords[0].Trigger)
	assert.NotNil(t, db.executionRecords[0].CompletionTime)
}
