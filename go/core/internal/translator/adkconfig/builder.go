...
func (c *Builder) ApplyCompaction(ctx context.Context, result *Result, harness *v1alpha3.Harness, template *v1alpha3.AgentTemplate) error {
	...
	if summarizer := spec.Summarizer; summarizer != nil {
		...
		if ref := summarizer.ModelConfigRef; ref != nil && !isAgentModel(template, ref.Name) {
			model, err := c.BuildModel(ctx, harness.Namespace, ref.Name)
			if err != nil {
