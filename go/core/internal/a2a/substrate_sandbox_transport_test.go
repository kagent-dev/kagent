...
func TestEnsureSessionRow(t *testing.T) {
	...
	t.Run("creates the row for a new session", func(t *testing.T) {
		...
		if row.AgentID == nil || *row.AgentID != "sandboxagents__NS__kagent__NS__my_agent" {
			t.Fatalf("expected session row bound to the sandbox agent's kind-qualified id, got %+v", row)
		}
	})
