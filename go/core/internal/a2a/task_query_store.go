...
// other-user session) surface to the caller. ListTasksForUser filters, orders,
...
type TaskStore interface {
	...
	ListTasksForUser(ctx context.Context, userID string, params dbpkg.ListTasksForUserParams) (tasks []*a2atype.Task, total int, err error)
}
...
func (h *storeTaskQueryHandler) ListTasks(ctx context.Context, req *a2atype.ListTasksRequest) (*a2atype.ListTasksResponse, error) {
	...
	// error, not an empty page. The join in ListTasksForUser also scopes by user, so
	...
	tasks, total, err := h.store.ListTasksForUser(ctx, userID, dbpkg.ListTasksForUserParams{
