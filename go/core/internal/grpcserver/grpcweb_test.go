...
func TestTrimAPIPrefix(t *testing.T) {
	tests := []struct {
		...
		{"a same-origin browser call arrives under /api", "/api/kagent.api.v1alpha1.SessionService/ListSessions", "/kagent.api.v1alpha1.SessionService/ListSessions", true},
		{"a client addressing the gRPC path directly is untouched", "/kagent.api.v1alpha1.SessionService/ListSessions", "/kagent.api.v1alpha1.SessionService/ListSessions", false},
