...
type CreateAgentHarnessRequest struct {
	AgentHarness *v1alpha2.AgentHarness
}
...
type Validator func(context.Context, v1alpha2.AgentObject) error

type ActorLifecycle interface {
	EnsureSessionActor(context.Context, *v1alpha2.AgentHarness, string) (sandboxbackend.EnsureResult, error)
	SuspendSessionActor(context.Context, *v1alpha2.AgentHarness, string) error
	GetSessionActorState(context.Context, *v1alpha2.AgentHarness, string) (substrate.SessionActorState, error)
}
...
func (s *Service) List(ctx context.Context, request ListRequest) ([]View, error) {
	...
	agents := &v1alpha2.AgentList{}
	if err := s.kubeClient.List(ctx, agents, options...); err != nil {
		return nil, serviceerrors.NewInternal("Failed to list Agents from Kubernetes", err)
	}
	sandboxAgents := &v1alpha2.SandboxAgentList{}
	...
	harnesses := &v1alpha2.AgentHarnessList{}
	...
	views := make([]View, 0, len(agents.Items)+len(sandboxAgents.Items)+len(harnesses.Items))
	for index := range agents.Items {
		view, _ := s.agentView(ctx, &agents.Items[index], KindAgent)
	...
	for index := range harnesses.Items {
		...
		if !v1alpha2.IsKnownAgentHarnessBackend(harness.Spec.Backend) {
			continue
		}
...
func (s *Service) GetAgent(ctx context.Context, request GetRequest) (View, error) {
	agent := &v1alpha2.Agent{}
	if err := s.get(ctx, request.Ref, agent, "Agent not found"); err != nil {
		return View{}, err
	}
	return s.agentView(ctx, agent, KindAgent)
}
...
func (s *Service) CreateAgent(ctx context.Context, request CreateAgentRequest) (View, error) {
	if request.Agent == nil {
		return View{}, serviceerrors.NewInvalidArgument("Agent resource is required", nil)
	}
	agent := request.Agent.DeepCopy()
	ref, err := s.prepareCreate(ctx, agent, KindAgent, auth.VerbCreate)
	if err != nil {
		return View{}, err
	}
	...
	if err := s.kubeClient.Create(ctx, agent); err != nil {
		if apierrors.IsAlreadyExists(err) {
			return View{}, serviceerrors.NewAlreadyExists("Agent already exists", err)
		}
		return View{}, serviceerrors.NewInternal("Failed to create Agent in Kubernetes", err)
	}
	view, _ := s.agentView(ctx, agent, KindAgent)
	view.Ref = ref
	return view, nil
}

func (s *Service) UpdateAgent(ctx context.Context, request UpdateAgentRequest) (View, error) {
	if request.Agent == nil {
		return View{}, serviceerrors.NewInvalidArgument("Agent resource is required", nil)
	}
	incoming := request.Agent.DeepCopy()
	ref, err := s.updateRef(incoming, request.Ref, false)
	if err != nil {
		return View{}, err
	}
	...
	if err := s.loadForMutation(ctx, ref, existing, "Agent not found", "Failed to get Agent"); err != nil {
		return View{}, err
	}
	...
	if err := s.kubeClient.Update(ctx, existing); err != nil {
		return View{}, serviceerrors.NewInternal("Failed to update Agent", err)
	}
	view, _ := s.agentView(ctx, existing, KindAgent)
	return view, nil
}

func (s *Service) DeleteAgent(ctx context.Context, request DeleteRequest) error {
	return s.delete(ctx, request.Ref, &v1alpha2.Agent{}, "Agent not found", "Failed to delete Agent")
}
...
func (s *Service) DeleteSandboxAgent(ctx context.Context, request DeleteRequest) error {
	return s.delete(ctx, request.Ref, &v1alpha2.SandboxAgent{}, "SandboxAgent not found", "Failed to delete SandboxAgent")
}
...
func (s *Service) CreateAgentHarness(ctx context.Context, request CreateAgentHarnessRequest) (View, error) {
	...
	if harness.APIVersion == "" {
		harness.APIVersion = v1alpha2.GroupVersion.String()
	}
	if harness.Kind == "" {
...
func (s *Service) validate(ctx context.Context, object v1alpha2.AgentObject) error {
	if sandboxAgent, ok := object.(*v1alpha2.SandboxAgent); ok {
		if err := v1alpha2.ValidateSubstrateSandboxAgentSpec(sandboxAgent); err != nil {
...
func (s *Service) actorHarness(ctx context.Context, request ActorRequest, verb auth.Verb) (ActorRequest, *v1alpha2.AgentHarness, error) {
	if request.Ref.Namespace == "" || request.Ref.Name == "" {
...
func normalizeSandboxAgent(agent *v1alpha2.SandboxAgent) {
	if agent.Spec.Type == "" {
		agent.Spec.Type = v1alpha2.AgentType_Declarative
	}
}
...
func (s *Service) agentView(ctx context.Context, object v1alpha2.AgentObject, kind Kind) (View, error) {
	...
	view := View{
		...
		WorkloadMode: object.GetWorkloadMode(),
	}
	for _, condition := range object.GetAgentStatus().Conditions {
		if condition.Type == "Ready" && condition.Status == metav1.ConditionTrue &&
			(condition.Reason == reconciler.AgentReadyReasonDeploymentReady || condition.Reason == reconciler.AgentReadyReasonWorkloadReady) {
			view.DeploymentReady = true
		}
...
func (s *Service) harnessView(ctx context.Context, harness *v1alpha2.AgentHarness) View {
	...
	for _, condition := range harness.Status.Conditions {
		if condition.Type == v1alpha2.AgentHarnessConditionTypeReady && condition.Status == metav1.ConditionTrue {
			view.DeploymentReady = true
		}
		if condition.Type == v1alpha2.AgentHarnessConditionTypeAccepted && condition.Status == metav1.ConditionTrue {
			view.Accepted = true
		}
	}
