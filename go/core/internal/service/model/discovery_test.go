...
func TestDiscoveryStaticModelCatalog(t *testing.T) {
	...
	require.NotEmpty(t, models[v1alpha2.ModelProviderOpenAI])
	assert.Equal(t, "gpt-5.6-terra", models[v1alpha2.ModelProviderOpenAI][0].Name)
	assert.True(t, models[v1alpha2.ModelProviderOpenAI][0].FunctionCalling)
	assert.Equal(t, model.ModelInfo{Name: "deepseek-r1", FunctionCalling: false}, models[v1alpha2.ModelProviderOllama][5])
	assert.Equal(t, model.ModelInfo{Name: "us.amazon.nova-2-lite-v1:0", FunctionCalling: false}, models[v1alpha2.ModelProviderBedrock][10])

	encoded, err := json.Marshal(models[v1alpha2.ModelProviderOpenAI][0])
...
func TestDiscoveryConfiguredProviders(t *testing.T) {
	...
	kubeClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(
		&v1alpha2.ModelProviderConfig{
			ObjectMeta: metav1.ObjectMeta{Name: "default-endpoint", Namespace: "service-ns"},
			Spec:       v1alpha2.ModelProviderConfigSpec{Type: v1alpha2.ModelProviderOpenAI},
			Status:     v1alpha2.ModelProviderConfigStatus{Conditions: readyCondition},
		},
		&v1alpha2.ModelProviderConfig{
		...
		&v1alpha2.ModelProviderConfig{
			...
			Spec:       v1alpha2.ModelProviderConfigSpec{Type: v1alpha2.ModelProviderAnthropic},
		},
		&v1alpha2.ModelProviderConfig{
...
func TestDiscoveryProviderModels(t *testing.T) {
	scheme := discoveryScheme(t)
	kubeClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(&v1alpha2.ModelProviderConfig{
		ObjectMeta: metav1.ObjectMeta{Name: "configured", Namespace: "service-ns"},
		Status: v1alpha2.ModelProviderConfigStatus{
