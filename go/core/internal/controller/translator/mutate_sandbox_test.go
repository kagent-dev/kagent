...
func TestMutateFuncFor_SandboxTemplate_replacesPodTemplateSlices(t *testing.T) {
	oldImg := "registry.example/old:v1"
	newImg := "registry.example/new:v2"

	existing := &extensionsv1alpha1.SandboxTemplate{
		ObjectMeta: metav1.ObjectMeta{Name: "kagent-a", Namespace: "ns"},
...
func TestMutateFuncFor_SandboxClaim_replacesSpec(t *testing.T) {
	existing := &extensionsv1alpha1.SandboxClaim{
		ObjectMeta: metav1.ObjectMeta{Name: "a", Namespace: "ns"},
		Spec: extensionsv1alpha1.SandboxClaimSpec{
			TemplateRef: extensionsv1alpha1.SandboxTemplateRef{Name: "t-old"},
		},
	}
	desired := &extensionsv1alpha1.SandboxClaim{
		ObjectMeta: metav1.ObjectMeta{Name: "a", Namespace: "ns"},
		Spec: extensionsv1alpha1.SandboxClaimSpec{
			TemplateRef: extensionsv1alpha1.SandboxTemplateRef{Name: "t-new"},
		},
	}

	f := MutateFuncFor(existing, desired)
	require.NoError(t, f())
	require.Equal(t, "t-new", existing.Spec.TemplateRef.Name)
}
