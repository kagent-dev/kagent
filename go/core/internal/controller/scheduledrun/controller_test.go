...
func TestControllerRecoversCleanupWithoutReplacingInstance(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cleanup.err = nil
			require.NoError(t, controller.reconcile(t.Context(), execution))
	...
	require.NoError(t, NewController(controllerTestStore{}, nil, nil).reconcile(t.Context(), execution))
...
func TestControllerKeepsCompletedOutcomeAfterDeadline(t *testing.T) {
	...
	require.NoError(t, NewController(store, nil, gateway).reconcile(t.Context(), execution))
