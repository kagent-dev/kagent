...
import (
	...
	"github.com/kagent-dev/kagent/go/cli/tui/dialogs"
	"github.com/kagent-dev/kagent/go/cli/tui/keys"
	"github.com/kagent-dev/kagent/go/cli/tui/theme"
...
func RunWorkspace(cfg *config.Config, clientSet *client.ClientSet, verbose bool, version string) error {
	m := newWorkspaceModel(cfg, clientSet, verbose, version)
...
func newWorkspaceModel(cfg *config.Config, clientSet *client.ClientSet, verbose bool, version string) *workspaceModel {
...
func (m *workspaceModel) resize() tea.Cmd {
	...
	headerLines := lineCount(m.renderTitle(m.width))
...
▀██               ██
  ▀███████████████`),
	...
	logo := lipgloss.NewStyle().Bold(true).Foreground(theme.ColorPrimary).Render(m.renderTitle(m.width))
...
func (m *workspaceModel) renderTitle(width int) string {
	_ = width
	title := fmt.Sprintf("kagent  %s", lipgloss.NewStyle().Foreground(theme.ColorMuted).Render(m.version))
	return title
}
