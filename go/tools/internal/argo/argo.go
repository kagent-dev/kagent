...
func RegisterArgoTools(s *server.MCPServer) {
	s.AddTool(mcp.NewTool("verify_argo_rollouts_controller_install",
		mcp.WithDescription("Verify Argo Rollouts controller is running in the kubernetes cluster"),
		mcp.WithString("namespace", mcp.Description("The namespace to check for the argo rollouts controller")),
		mcp.WithString("label", mcp.Description("The label to check for the argo rollouts controller")),
	), handleVerifyArgoRolloutsControllerInstall)

	s.AddTool(mcp.NewTool("verify_kubectl_plugin_install",
		mcp.WithDescription("Verify Argo Rollouts kubectl plugin installation status"),
	), handleVerifyKubectlPluginInstall)

	s.AddTool(mcp.NewTool("promote_rollout",
		mcp.WithDescription("Promote a rollout in Argo Rollouts"),
		...
		mcp.WithString("full", mcp.Description("Perform a full promotion, skipping analysis, pauses, and steps (true/false)")),
	), handlePromoteRollout)

	s.AddTool(mcp.NewTool("pause_rollout",
		mcp.WithDescription("Pause a rollout in Argo Rollouts"),
		mcp.WithString("rollout_name", mcp.Description("The name of the rollout to pause"), mcp.Required()),
		mcp.WithString("namespace", mcp.Description("The namespace of the rollout")),
	), handlePauseRollout)

	s.AddTool(mcp.NewTool("set_rollout_image",
		mcp.WithDescription("Set the image for a container in an Argo Rollouts deployment"),
		mcp.WithString("rollout_name", mcp.Description("The name of the rollout to update"), mcp.Required()),
		mcp.WithString("container_image", mcp.Description("Container name and image in the format 'container=image'"), mcp.Required()),
		mcp.WithString("namespace", mcp.Description("The namespace for the resource")),
	), handleSetRolloutImage)

	s.AddTool(mcp.NewTool("verify_gateway_plugin",
		mcp.WithDescription("Verify and configure Gateway API plugin for Argo Rollouts"),
		mcp.WithString("version", mcp.Description("Version of the Gateway API plugin to verify")),
		mcp.WithString("namespace", mcp.Description("Namespace where Argo Rollouts is installed")),
		mcp.WithString("should_install", mcp.Description("Flag to determine if the plugin should be installed if not present (true/false)")),
	), handleVerifyGatewayPlugin)

	s.AddTool(mcp.NewTool("check_plugin_logs",
		mcp.WithDescription("Check Argo Rollouts controller logs for Gateway API plugin installation status"),
		mcp.WithString("namespace", mcp.Description("Namespace where Argo Rollouts is installed")),
		mcp.WithString("timeout", mcp.Description("Timeout in seconds for log checking")),
