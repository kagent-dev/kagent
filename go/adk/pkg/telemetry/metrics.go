...
const (
	...
	labelGenAIAgentName     = "gen_ai_agent_name"
	...
	metricsEnabledEnvVar = "OTEL_METRICS_ENABLED"
)
...
// standard Go/process collectors. It is always registered; MetricsEnabled
// gates whether observations are recorded, keeping the scrape endpoint stable.
...
// MetricsEnabled reports whether the GenAI token metrics pipeline is on, i.e.
// the OTEL_METRICS_ENABLED gate used consistently across kagent's runtimes.
func MetricsEnabled() bool {
	return strings.EqualFold(strings.TrimSpace(os.Getenv(metricsEnabledEnvVar)), "true")
}
...
type TokenUsage struct {
	...
	Provider string
	// AgentName is gen_ai.agent.name (the agent that produced the tokens).
	AgentName string
...
func RecordTokenUsage(u TokenUsage) {
	...
	if u.CachedTokens > 0 {
		tokenUsage.WithLabelValues(tokenTypeCached, operationChat, u.Provider, u.RequestModel, responseModel, u.AgentName, u.ErrorType).
			Observe(float64(u.CachedTokens))
	}
}
