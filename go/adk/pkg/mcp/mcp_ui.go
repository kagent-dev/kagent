...
// HasUIResource reports whether MCP metadata points at a UI resource
// (`_meta.ui.resourceUri`, or the flat `_meta["ui/resourceUri"]` form). Callers
// use it to tell a result that actually carries an MCP App render payload from
// one produced by an App-capable tool whose call returned ordinary content.
