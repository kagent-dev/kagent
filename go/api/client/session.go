...
func (c *sessionClient) ListSessionRuns(ctx context.Context, sessionName string) (*api.StandardResponse[any], error) {
	...
	for _, value := range response.GetTasks() {
		task := &a2a.Task{}
		if err := structuredobject.ToGo(value, a2aTaskKind, task, c.client.grpc.maxMessageBytes); err != nil {
...
func (c *BaseClient) taskServiceClient() (apiv1alpha1.TaskServiceClient, error) {
	connection, err := c.grpcConnection()
	if err != nil {
		return nil, err
	}
	return apiv1alpha1.NewTaskServiceClient(connection), nil
}
