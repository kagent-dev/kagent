...
def test_model_validate_handles_zip_mismatch():
    """Test that model_validate handles cases where http_tools_data and instance.http_tools have different lengths."""
    # This is an edge case - if Pydantic validation somehow creates a different number of http_tools
    # than in the original JSON, zip() would silently truncate. Let's test this doesn't cause issues.
    config_dict = {
        "model": {
            "type": "openai",
            "model": "gpt-4",
