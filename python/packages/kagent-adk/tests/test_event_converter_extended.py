...
class TestProcessLongRunningTool:
    ...
    def test_non_function_call_part(self):
        ...
        data_part = DataPart(
            data={"data": "some data"},
            metadata={
                get_kagent_metadata_key(A2A_DATA_PART_METADATA_TYPE_KEY): "OTHER_TYPE"
            }
        )
        a2a_part = Part(root=data_part)
...
class TestEdgeCases:
    ...
    def test_metadata_with_partial_optional_fields(self):
        ...
        assert get_kagent_metadata_key("grounding_metadata") not in result or \
               result.get(get_kagent_metadata_key("grounding_metadata")) is None
