    ...
    def to_agent(self, name: str) -> Agent:
        ...
        if self.remote_agents:
            for remote_agent in self.remote_agents:  # Add remote agents as tools
                ...
                mcp_toolsets.append(AgentTool(agent=remote_agent, skip_summarization=True))        # Get headers from model config
        ...
        if self.model.type == "openai":
            ...
            model = LiteLlm(model=f"anthropic/{self.model.model}", base_url=self.model.base_url, extra_headers=extra_headers)
