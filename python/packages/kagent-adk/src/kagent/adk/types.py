...
def create_header_provider(
    ...
    sts_header_provider: Callable[[Optional[ReadonlyContext], Optional[str]], dict[str, str]] | None = None,
    ...
    Args:
        ...
        sts_header_provider: Optional STS header provider for token propagation.
                            ...
                            Signature: (readonly_context, audience) -> headers
        sts_audience: Optional audience value for STS token scoping for this toolset.
                     Passed to the STS header provider to retrieve the correct
    ...
    def header_provider(readonly_context: Optional[ReadonlyContext]) -> dict[str, str]:
        ...
        if sts_header_provider:
            sts_headers = sts_header_provider(readonly_context, sts_audience)
            if sts_headers:
...
class HttpMcpServerConfig(BaseModel):
    ...
    sts_audience: str | None = None  # Audience for STS token exchange scoping
        ...
        if self.http_tools:
            for http_tool in self.http_tools:  # add http tools
                ...
                tool_header_provider = create_header_provider(
                    ...
                    sts_audience=http_tool.sts_audience,
                )
                ...
                if sts_integration and http_tool.sts_audience:
                    sts_integration.register_toolset(toolset, http_tool.sts_audience)
        ...
        if self.sse_tools:
            for sse_tool in self.sse_tools:  # add sse tools
                ...
                tool_header_provider = create_header_provider(
                    ...
                    sts_header_provider=sts_header_provider,
                    sts_audience=sse_tool.sts_audience,
                )
                toolset = KAgentMcpToolset(
                ...
                tools.append(toolset)
                if sts_integration and sse_tool.sts_audience:
                    sts_integration.register_toolset(toolset, sse_tool.sts_audience)
