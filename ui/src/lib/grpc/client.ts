...
function getTaskClient(): TaskClient {
  ...
  if (cachedTaskClient === undefined || cachedTaskTarget !== target) {
    ...
    cachedTaskClient = createClient(TaskService, transport);
...
export class AgentGrpcGateway {
  ...
  async getAgent(namespace: string, name: string, kind: AgentKubernetesKind = "Agent"): Promise<AgentResponse> {
    ...
    switch (kind) {
      ...
      default: {
        const response = await this.call(() => this.client.getAgent({ ref }, this.options()));
        return agentFromMessage(requiredAgent(response.agent));
      }
    }
  }

  async createAgent(resource: Agent): Promise<AgentResponse> {
    const response = await this.call(() => this.client.createAgent({
      ref: resourceReference(resource),
      resource: structuredAgentResource(resource, "Agent"),
    }, this.options()));
    return agentFromMessage(requiredAgent(response.agent));
  }

  async updateAgent(resource: Agent): Promise<AgentResponse> {
    const response = await this.call(() => this.client.updateAgent({
      ref: resourceReference(resource),
      resource: structuredAgentResource(resource, "Agent"),
    }, this.options()));
    return agentFromMessage(requiredAgent(response.agent));
  ...
  async deleteAgent(namespace: string, name: string, kind: AgentKubernetesKind = "Agent"): Promise<void> {
    const ref = { namespace, name };
    switch (kind) {
      case "SandboxAgent":
      ...
      case "AgentHarness":
        ...
        return;
      default:
        await this.call(() => this.client.deleteAgent({ ref }, this.options()));
    }
  }
...
export class SessionGrpcGateway {
  ...
  async listTasks(sessionId: string, shareToken?: string): Promise<Task[]> {
    const response = await this.call(() => this.taskClient.listTasks(
      { sessionId },
      this.options(shareToken),
    ));
    return response.tasks.map((task) => requiredStructuredTask(task.value));
  }
...
function requiredStructuredTask(value: JsonObject | undefined): Task {
  if (!isJsonObject(value)) {
    throw new Error("TaskService response did not include a valid A2A Task");
  }

  const status = requiredJsonObject(value.status, "Task status");
  const normalized: JsonObject = {
    ...value,
    kind: "task",
    status: normalizeA2ATaskStatus(status),
  };
  ...
  if (value.history !== undefined) {
    ...
    normalized.history = value.history.map((message) => normalizeA2AMessage(message));
  }
  if (value.artifacts !== undefined) {
    if (!Array.isArray(value.artifacts)) {
      throw new Error("Task artifacts must be an array");
    }
    normalized.artifacts = value.artifacts.map((artifact) => normalizeA2AArtifact(artifact));
  }

  return normalized as unknown as Task;
}

function normalizeA2ATaskStatus(value: JsonObject): JsonObject {
  const normalized: JsonObject = {
    ...value,
    state: normalizeA2ATaskState(value.state),
  };
  if (value.message !== undefined) {
    normalized.message = normalizeA2AMessage(value.message);
  }
  return normalized;
}

function normalizeA2AMessage(value: unknown): JsonObject {
  const message = requiredJsonObject(value, "Task message");
  if (!Array.isArray(message.parts)) {
    throw new Error("Task message parts must be an array");
  }
  return {
    ...message,
    kind: "message",
    role: normalizeA2AMessageRole(message.role),
    parts: message.parts.map((part) => normalizeA2APart(part)),
  };
}

function normalizeA2AArtifact(value: unknown): JsonObject {
  const artifact = requiredJsonObject(value, "Task artifact");
  if (!Array.isArray(artifact.parts)) {
    throw new Error("Task artifact parts must be an array");
  }
  return {
    ...artifact,
    parts: artifact.parts.map((part) => normalizeA2APart(part)),
  };
}

function normalizeA2APart(value: unknown): JsonObject {
  const part = requiredJsonObject(value, "Task content part");
  ...
  const hasUrl = typeof part.url === "string" && part.url !== "";
  const hasRaw = typeof part.raw === "string";
  const contentFieldCount = [hasText, hasData, hasUrl, hasRaw].filter(Boolean).length;
  if (contentFieldCount !== 1) {
    throw new Error(`Task content part must have exactly one content field; received ${contentFieldCount}`);
  }

  if (hasText) {
    const normalized: JsonObject = { kind: "text", text: part.text };
    if (isJsonObject(part.metadata)) {
      normalized.metadata = part.metadata;
    }
    return normalized;
  }
  if (hasData) {
    const normalized: JsonObject = { kind: "data", data: part.data };
    if (isJsonObject(part.metadata)) {
      normalized.metadata = part.metadata;
    }
    return normalized;
  ...
  const normalized: JsonObject = { kind: "file", file };
  if (isJsonObject(part.metadata)) {
    normalized.metadata = part.metadata;
  }
  return normalized;
...
function normalizeA2AMessageRole(value: unknown): "user" | "agent" {
  switch (value) {
    case "ROLE_USER":
    case "user":
      return "user";
    ...
    case "agent":
      return "agent";
    default:
      throw new Error(`Unsupported A2A message role: ${String(value)}`);
  }
}

function normalizeA2ATaskState(value: unknown): string {
  ...
  if (typeof value !== "string") {
    throw new Error(`Unsupported A2A task state: ${String(value)}`);
  }
  ...
  if (sdkStates.has(value)) {
    return value;
  }
  throw new Error(`Unsupported A2A task state: ${value}`);
}

function requiredJsonObject(value: unknown, description: string): JsonObject {
  if (!isJsonObject(value)) {
...
function structuredAgentResource(resource: AgentResourceInput, kind: AgentKubernetesKind) {
  return {
    apiVersion: resource.apiVersion || "kagent.dev/v1alpha2",
    kind,
...
function agentFromMessage(message: AgentMessage): AgentResponse {
  ...
  const agent: Agent = kind === "AgentHarness"
    ? {
        apiVersion: message.resource?.apiVersion || "kagent.dev/v1alpha2",
        kind,
        metadata,
        spec: {
  ...
  const workloadMode = message.workloadMode === WorkloadMode.DEPLOYMENT
    ? "deployment"
    : message.workloadMode === WorkloadMode.SANDBOX
      ? "sandbox"
      : undefined;
  ...
  return {
    ...
    modelConfigRef,
    tools,
    deploymentReady: message.deploymentReady,
    accepted: message.accepted,
    ...(workloadMode === undefined ? {} : { workloadMode }),
