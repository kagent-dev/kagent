...
describe("gRPC client", () => {
  ...
  it("builds AgentService mutation payloads and maps actor lifecycle states", async () => {
    ...
    const createAgent = jest.fn().mockResolvedValue({ agent: agentMessage });
    ...
    await gateway.createAgent(agentMessage.resource.value as never);
      ...
      tasks: [{
        value: {
          ...
          history: [{
            messageId: "message-1",
            role: "ROLE_USER",
            parts: [{ text: "hello", data: { duplicate: true } }],
          }],
        },
      }],
    });
