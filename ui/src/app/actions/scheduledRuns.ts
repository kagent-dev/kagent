...
import { ScheduledRun, RunHistoryEntry, BaseResponse } from "@/types";
...
export async function triggerScheduledRun(name: string, namespace: string): Promise<BaseResponse<RunHistoryEntry>> {
  try {
    const response = await fetchApi<BaseResponse<RunHistoryEntry>>(`/scheduledruns/${namespace}/${name}/trigger`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    revalidatePath("/schedules");
    return { message: "Successfully triggered scheduled run", data: response.data };
  } catch (error) {
    return createErrorResponse<RunHistoryEntry>(error, "Error triggering scheduled run");
  }
}
