...
export async function POST(
  ...
  try {
    ...
    const targetUrl = `${backendUrl}/a2a-sandboxes/${namespace}/${agentName}/`;
          ...
          return reader.read().then(({ done, value }): Promise<void> => {
            if (done) {
              if (!isClosed) {
                ...
                controller.close();
                isClosed = true;
                console.log('A2A Sandbox Proxy: Backend connection closed.');
              }
            ...
            console.error('A2A Sandbox Proxy: Error in stream pump:', error);
            if (keepAliveTimer) clearTimeout(keepAliveTimer);
