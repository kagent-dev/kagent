...
function executionKey(execution: ScheduledRunExecution): string {
  return execution.id;
}
...
export default function ScheduledRunDetailPage() {
  ...
  const fetchData = useCallback(async (showLoading = true) => {
    try {
      ...
      if (showLoading) {
        setHasMoreExecutions(Boolean(executionsResponse.data?.length === EXECUTIONS_PAGE_SIZE));
      }
      setExecutions((existing) => {
        if (showLoading || existing.length === 0) {
          return latestExecutions;
        }
        return mergeExecutions(latestExecutions, existing);
      });
      setError(null);
    } catch (err) {
  ...
  const handleLoadMoreExecutions = async () => {
    ...
    const beforeID = oldest && "id" in oldest && typeof oldest.id === "string" ? oldest.id : undefined;
    ...
    try {
      ...
      setExecutions((existing) => mergeExecutions(existing, older));
