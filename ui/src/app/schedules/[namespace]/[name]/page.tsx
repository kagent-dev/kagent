...
export default function ScheduledRunDetailPage() {
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      // outcome.
      if (response.data?.dispatchStatus === "DispatchFailed") {
        toast.error(`Dispatch failed: ${response.data.dispatchMessage ?? "agent dispatch error"}`);
      } else {
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex space-x-2">
            ...
            <Button
              ...
              title={isTriggering ? "Dispatching to agent — outcome appears in run history" : undefined}
            >
              {isTriggering ? (
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
              ) : (
