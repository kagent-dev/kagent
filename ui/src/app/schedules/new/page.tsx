...
function BasicInformationSection({ state, setState, errors, isFormDisabled, isEditMode }: SectionProps) {
  return (
    <Card>
      <CardHeader>
...
function ScheduleConfigurationSection({ state, setState, errors, isFormDisabled, isEditMode, agents }: SectionProps & { agents: AgentResponse[] }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-xl font-bold">
...
function AdvancedSettingsSection({ state, setState, errors, isFormDisabled }: SectionProps) {
  return (
    <Card>
      <CardHeader>
...
function ScheduledRunFormContent() {
  ...
  const prefilledAgentName = searchParams.get("agentName") ?? "";
  const prefilledAgentNamespace = searchParams.get("agentNamespace") ?? "";
  const prefilledAgentKind = (searchParams.get("agentKind") ?? "Agent") as ScheduledRunTargetKind;

  const [agents, setAgents] = useState<AgentResponse[]>([]);
  const [state, setState] = useState<FormState>({
    name: "",
    namespace: prefilledAgentNamespace,
    schedule: "",
    timeZone: "UTC",
    targetName: prefilledAgentName,
    targetNamespace: prefilledAgentNamespace,
    targetKind: prefilledAgentKind,
    prompt: "",
    suspended: false,
