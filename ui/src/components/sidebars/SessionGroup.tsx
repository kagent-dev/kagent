          ...
          <CollapsibleContent>
            <SidebarMenuSub className="mx-0 px-0 ml-2 pl-2">
              {sessions.map((session) => (
                <ChatItem key={session.id} sessionId={session.id!} agentName={agentName} agentNamespace={agentNamespace} onDelete={onDeleteSession} sessionName={session.name} onDownload={onDownloadSession} createdAt={session.updated_at || session.created_at} hideDelete={hideSessionDelete} />
              ))}
            </SidebarMenuSub>
          </CollapsibleContent>
        </Collapsible>
