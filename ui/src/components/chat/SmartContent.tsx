...
function ValueRenderer({ value, className, depth = 0 }: RendererProps & { value: unknown }) {
  ...
  if (typeof value === "string") {
    return <StringRenderer content={value} className={className} depth={depth} />;
  }
  ...
  if (Array.isArray(value)) {
    ...
    return (
      <div className={`space-y-1 ${className ?? ""}`}>
        {value.map((item, i) => {
          const isPrimitive = typeof item === "string" || typeof item === "number" || typeof item === "boolean";
  ...
  if (typeof value === "object") {
    return <ObjectRenderer obj={value as Record<string, unknown>} className={className} depth={depth} />;
  }

  return <span className="text-sm">{String(value)}</span>;
}

function StringRenderer({ content, className, depth = 0 }: RendererProps & { content: string }) {
  const parsed = tryParseJson(content);
  if (parsed !== null && typeof parsed === "object") {
    return <ValueRenderer value={parsed} className={className} depth={depth + 1} />;
  }
...
function ObjectRenderer({ obj, className, depth = 0 }: RendererProps & { obj: Record<string, unknown> }) {
  ...
  return (
    <div className="relative overflow-hidden">
      <div className="absolute top-0 right-0 flex items-center gap-0.5 z-10">
        <Button variant="ghost" size="icon" className="h-5 w-5" onClick={handleToggleSource} title={viewSource ? "Formatted view" : "View source"} aria-label={viewSource ? "Formatted view" : "View source"}>
          {viewSource ? <Eye className="w-3 h-3" /> : <Code className="w-3 h-3" />}
        </Button>
        <Button variant="ghost" size="icon" className="h-5 w-5" onClick={handleCopy} title={copied ? "Copied!" : "Copy to clipboard"} aria-label={copied ? "Copied!" : "Copy to clipboard"}>
...
export function parseJsonOrString(content: string): unknown {
