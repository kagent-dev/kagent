...
// statusBadge maps the (DispatchStatus, Outcome) pair to what we surface in
// the Status column. Dispatched + Outcome=Succeeded is a true success. A
// pending outcome shows as "Running" so users don't read it as completed.
function statusBadge(entry: RunHistoryEntry): { label: string; variant: "default" | "destructive" | "secondary" | "outline"; className: string } {
  if (entry.dispatchStatus === "DispatchFailed") {
  ...
  switch (entry.outcome) {
    ...
    default:
      return { label: "Dispatched", variant: "outline", className: "" };
  }
}
    ...
    <Table>
      ...
      <TableBody>
        {sorted.map((entry, index) => {
          ...
          const message = entry.outcomeMessage || entry.dispatchMessage || "";
          return (
