              ...
              <TableCell className="font-mono text-xs">
                {execution.sessionID ? (
                  targetNamespace && targetName ? (
                    <a
                      href={`/agents/${encodeURIComponent(targetNamespace)}/${encodeURIComponent(targetName)}/chat/${encodeURIComponent(execution.sessionID)}`}
                      className="text-blue-500 hover:underline"
                      aria-label={`Open session ${execution.sessionID}`}
                      title={execution.sessionID}
                    >
                      {execution.sessionID.slice(0, 8)}...
                    </a>
                  ) : (
                    <span title={execution.sessionID}>{execution.sessionID.slice(0, 8)}...</span>
                  )
                ) : (
                  "-"
                )}
